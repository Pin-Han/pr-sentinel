...
**LangGraph graph** (`src/agent/graph.py`): Nodes receive external dependencies (GitHub client, Anthropic client) via `functools.partial`. Each node returns only the state keys it updates, not the full state.
...
**LLM integration**: `analyze_code` calls the Anthropic API directly (not via LangChain wrapper) using `tool_use` with forced tool choice to guarantee structured JSON output. The tool schema is defined in `src/agent/prompts.py`.
...
Required env vars: `ANTHROPIC_API_KEY`, `GITHUB_TOKEN`, `GITHUB_WEBHOOK_SECRET`. See `.env.example`.
